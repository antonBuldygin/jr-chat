 ...
 *   2. Если удачно — разблокировать и вернуть текст обратно, очистить форму и
 * отобразить обновленный список сообщений
 *   3. Если ошибка — разблокировать и вернуть текст обратно, не сбрасывать
 * форму и показать ошибку
 * 3. Optimistic UI
 *   1. Мгновенно обновляет список сообщений и показывает наше сообщение в
 * списке Очищает форму и дает возможность отправить новое сообщение Вновь
 * созданному сообщению добавляет визуальный индикатор о его состоянии
 *
 *
 *
 *
 ...
 */
{
    ...
    // Форма может жить в двух состояниях — модальное окно показано и модальное
    // окно не показано Режим когда окно не показано может быть инициализирован
    // после того как  имя пользователя было введено При создании функционала
    // некоего модуля, который описывает работу с DOM, нужно описывать не
    // только инициализацию, но и "разрушение" этого модуля
